...
**Balance:** The bar is triviality, not repetition. A one-off still earns a lever when the lever is what makes the work checkable. Follow `brain/principles/subtract-before-you-add.md`: build the smallest script that does or proves the job, never a framework.

This differs from `brain/principles/encode-lessons-in-structure.md`, which turns a recurring instruction into a durable guardrail. Build the lever for throughput and reviewability on the current work. Use `brain/principles/prove-it-works.md` when scripting the verification itself.
