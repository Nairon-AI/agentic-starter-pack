...
- **Grilling** (HITL): Conversation via `/grill-me`, which invokes domain modeling when needed. The default case.
...
1. **Name the destination.** Run `/grill-me` to pin down what this map is finding its way to — the spec, decision, or change. The destination fixes the scope, so it's settled first.
