...
license: Apache 2.0-derived guidance. See /NOTICE.md for attribution details.
---
...
2. **Check repo-local context (fast)**: Read `project-context/design-context.md` if it exists. Then check `AGENTS.md` for a `## Design Context` section.
3. **Run teach-impeccable (REQUIRED)**: If neither source has the required context, you MUST load `teach-impeccable` before doing anything else. Do NOT skip this step. Do NOT attempt to infer the missing context from the codebase instead.
