...
Write each entry the way you would tell a teammate what you did. Plain words, concrete actions, no AI jargon. Apply `stop-slop` to log text. A reviewer should understand each row without decoding it.
...
- Prefer evidence produced by reusable scripts over hand-made one-offs so a reviewer can rerun it. Follow `brain/principles/encode-lessons-in-structure.md`.

## Audit the log against the transcript

At the end of the run, check that the log tells the truth. Use the current run's workspace-scoped transcript or tool trace when the host exposes one. Never search unrelated project or home-directory history. When no transcript exists, audit against the current conversation, command outputs, diffs, commits, and artifacts. Walk the log against what actually happened:
...
Before handing back, spawn a fresh read-only reviewer. Prefer a different model family when the host supports model selection. The reviewer reads the audit trail and available run evidence, then flags what the user should inspect. If subagents are unavailable, perform the same audit yourself and disclose that independent review was unavailable.
...
Every reply for a run that produced a trail ends with an "Attention" section. Name the reviewer label, then list each flag with specific rows or moments. "No flags" is valid. The self-audit asks whether the log told the truth; this pass asks what the user should still scrutinize.
