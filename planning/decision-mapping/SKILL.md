...
- **Discuss**: Conversation with the agent. Uses `/grill-me`, which invokes domain modeling when needed. The default case.
...
1. Run `/grill-me` to surface the open decisions and maintain the domain model when needed.
...
2. Run a session to resolve the ticket, invoking skills as needed. If in doubt, use `/grill-me`.
