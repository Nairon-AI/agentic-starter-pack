...
Historical context spreads across seven evidence categories: source control history, issue or ticket tracking, long-form documents, real-time team chat, infrastructure observability, error or exception tracking, and product analytics warehouses. Discover the connectors, MCPs, apps, and local tools available at run time, map them to categories, then investigate at a depth proportional to the question. A narrow repository-history question with direct evidence may need only source control and linked records; a broad, high-stakes, contradictory, or weakly documented question warrants every relevant available category. Null results from searched categories are first-class evidence; report them alongside positive findings.
...
If the target is vague, infer it from conversation context, active files when available, and recent edits. State the interpretation briefly so the user can redirect, then proceed.
...
## Step 3. Run Proportional Investigations

Choose scope after building the code anchor. Start with source control and any systems directly linked from commits, PRs, comments, or ticket IDs. Expand when the initial evidence is indirect, contradictory, incomplete, or points to another category.

Use the full relevant-category sweep when the question is broad, high-stakes, incident-related, about a data-backed threshold, or explicitly asks for exhaustive archaeology. Do not query unrelated external systems merely to manufacture null results.

### Discovery

Before spawning investigators, inspect the host's available tool and skill map. Include connected apps, MCPs, repository tools, and local CLIs. Follow each connector's own skill or usage instructions; do not assume an unavailable discovery API exists.
...
Record a **coverage map** of sources searched, unavailable, and deliberately out of scope. A null result from a searched issue tracker is evidence the decision was not found there; document the null without overstating what it proves.

Run independent categories concurrently when capacity permits. Respect the host's global concurrency limit: use available slots in waves, or investigate directly while preserving a separate result per category. Do not let orchestration overhead exceed the investigation.

Subagent posture:
- Prefer one fresh investigator per selected category when capacity permits.
- Keep the task read-only, but use an agent mode that preserves connector access.
- Prefer a fast model for retrieval when the host supports model selection.
- If subagents are unavailable, run category searches sequentially and preserve the same coverage map.
...
### Investigator roster. Select from these evidence categories

For a full sweep, assign one investigator per selected category. For a narrow investigation or constrained host, the parent may own one or more categories directly, but must keep their searches and findings distinct.
...
For every category that could reasonably contain the answer, either search it or give an **explicit, written justification** in the final "Sources Consulted" section. Valid reasons include:
...
- **The question is narrow and direct evidence already answers it.** Name the evidence and why the omitted category would not materially change confidence. Example: "Product analytics not searched. The question concerns a repository layout migration explicitly described by the introducing commit, not runtime behavior or a data-derived threshold."
...
If a single commit or PR explicitly contains the complete answer, answer inline after checking its linked records and state the narrower scope. Expand if the user asks for exhaustive coverage or the direct record conflicts with later evidence.

## Step 4. Synthesize

Spawn one fresh synthesizer subagent in a mode that preserves connector access for citation spot-checking. Prefer a strong judgment model when selection is available. If subagents are unavailable, synthesize directly and spot-check citations yourself.
...
- **Skipping relevant investigators by anticipation**. Deciding that a source "probably" has nothing without tying the omission to the target, evidence, and confidence needed. A null result is a data point; an unjustified skip is a blind spot.
- **Collapsing evidence categories**. Each system has its own query vocabulary, result shape, and pitfalls. Even when the parent handles several categories because of host limits, keep each category's search and findings separate.
